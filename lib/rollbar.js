...
function Rollbar(tokens) {
   ...
   *
   * @param {String} teamId - Team ID of the team about which to return information
   * @param {requestCallback} callback
   *
   ...
   *
   * @param {Object} [options] - Optional object to access pagination in results
   * @param {String | Integer} options.page - The Rollbar API will list 20 teams per page; view more results by increasing the page number
   * @param {requestCallback} callback
   *
   * @see https://rollbar.com/docs/api/teams/#list-your-teams
   */
  this.listTeams = function (options, callback) {
    var passedOptions;
    ...
    var requestObject = ApiRequest.builder()
      .withPath('/teams')
      .withQueryParameters(passedOptions)
      .build();
  ...
  this.createTeam = function (options, callback) {
    if (typeof options !== 'object') {
      throw new Error('Options must be an object containing parameters name and access_level.');
    }
   ...
   */
  this.deleteTeam = function (teamId, callback) {
    var requestObject = ApiRequest.builder()
      .withPath('/team/' + teamId)
      .build();
   ...
   */
  this.isProjectInTeam = function (teamId, projectId, callback) {
    var requestObject = ApiRequest.builder()
   ...
   * @param {String | Integer}  teamId - Team ID of the team to add to a project
   * @param {String | Integer}  projectId - Project ID of the project to which the team will be added
   * @param {requestCallback} callback
   *
   ...
   *
   * @param {String | Integer} teamId - Team ID of the team from which to remove the project
   * @param {String | Integer} projectId - Project ID of the project to remove from the team
   * @param {requestCallback} callback
   *
   * @see https://rollbar.com/docs/api/teams/#remove-a-project-from-a-team
   */
  this.removeProjectFromTeam = function (teamId, projectId, callback) {
    var requestObject = ApiRequest.builder()
      .withPath('/team/' + teamId + '/project/' + projectId)
      .build();

    _addWriteToken(requestObject);
    HttpRequest.delete(requestObject, callback);
  };

  /**
   * Check team membership
   *
   * @param {String | Integer} teamId - Team ID to check for membership
   * @param {String | Integer} userId - User ID to check for membership
   * @param {requestCallback} callback
   *
   * @see https://rollbar.com/docs/api/teams/#check-team-membership
   */
  this.checkTeamMembership = function (teamId, userId, callback) {
    var requestObject = ApiRequest.builder()
      .withPath('/team/' + teamId + '/user/' + userId)
      .build();
   ...
   * List team members
   *
   * @param {String | Integer} teamId - Team ID for which to list members
   * @param {Object} [options] - Optional object for pagination settings
   * @param {Integer} options.page - The Rollbar API lists 5000 results by default. Increase the page number to list more
   * @param {requestCallback} callback
   *
   * @see https://rollbar.com/docs/api/teams/#list-team-members
   */
  this.listTeamMembers = function (teamId, options, callback) {
    var passedOptions;
   ...
   *
   * @param {String | Integer} teamId - Team ID of the team from which to remove a user
   * @param {String | Integer} userId - User ID of the user to remove from a team
   * @param {requestCallback} callback
   *
   * @see https://rollbar.com/docs/api/teams/#remove-a-user-from-a-team
   */
  this.removeUserFromTeam = function (teamId, userId, callback) {
    var requestObject = ApiRequest.builder()
      .withPath('/team/' + teamId + '/user/' + userId)
      .build();
   ...
   *
   * @param {String | Integer} userId - User ID for the user about which to get information
   * @param {requestCallback} callback
   *
