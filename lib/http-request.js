...
var _formatError = function (response, body) {
	...
	return new ApiError(message, response.statusCode, response.statusMessage);
};
...
HttpRequest._makeRequest = function (method, options, callback) {
	method(options, function (error, response, body) {
		...
		if (response.statusCode > 200) {
			...
			return callback(apiErrorObject);
		}
		...
		if (typeof body !== "object") {
			try {
				...
				return callback(new Error("Invalid JSON response from server."));
			}
		} else {
			result = body;
		}
...
HttpRequest.delete = HttpRequest.del;
